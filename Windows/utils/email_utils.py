...
import imaplib
import email
from Windows.configurations.email_configurations import EMAIL_PASSWORD, EMAIL_ADDRESS, SMTP_SERVER, IMAP_SERVER
from email.mime.text import MIMEText
from email.mime.multipart import MIMEMultipart
from email.mime.image import MIMEImage
import cv2
from Windows.configurations.email_configurations import TO_EMAILS
    ...
    for email in TO_EMAILS:
        try:
            with imaplib.IMAP4_SSL(IMAP_SERVER) as mail:
                ...
                result, data = mail.search(None, f'(FROM "{email}")')
...
def delete_all_emails_from_sender():
    try:
        with imaplib.IMAP4_SSL(IMAP_SERVER) as mail:
            mail.login(EMAIL_ADDRESS, EMAIL_PASSWORD)
            mail.select("inbox")  # Or "ALL" for all folders, but "inbox" is typical
...
for emails in TO_EMAILS:
    ...
    with imaplib.IMAP4_SSL(IMAP_SERVER) as mail:
        ...
        result, data = mail.search(None, f'(FROM "{TO_EMAILS}")')
