from utils.email_utils import check_for_user_response, send_email
...
if TEST_IMAGE:
    print("WARNING, test image is enabled, camera won't be used!")
if not SEND_EMAIL:
    print("WARNING, email sending is disabled!")
if not CHECK_PRINTING_STATUS:
    print("WARNING, printer printing status won't be checked!")
if not CHECK_EMAILS:
    print("WARNING, email checking is disabled!")
...
class PrinterMonitor:
    ...
    def is_camera_available(self):
        ...
        cap.release()
        if not ret:
            print(f"[{self.printer_name}] Camera is not available, check camera url")
    ...
    def check_email_reply(self, printer_name):
        if CHECK_EMAILS:
            return check_for_user_response(self.printer_name)
        else:
            return True
